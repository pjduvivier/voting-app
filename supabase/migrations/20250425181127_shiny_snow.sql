/*
  # Final RLS policy fix for photos table

  1. Changes
    - Drop all existing policies to start fresh
    - Re-enable RLS
    - Create comprehensive policies with proper checks
    
  2. Security
    - Allow public read access to all photos
    - Allow authenticated users to insert photos with field validation
...
WITH CHECK (
  ...
  url ~ '^https?://'  -- Ensure URL starts with http:// or https://
);
...
WITH CHECK (
  ...
  (
    CASE 
      WHEN votes IS NOT NULL AND 
           votes >= 0  -- Ensure votes can't be negative
      THEN true
      ELSE false
    END
  )
